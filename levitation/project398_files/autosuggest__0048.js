...
window.log=function(a,id,path)
{try{if(a===window){a=window.event.srcElement;while(a){if(a.href)
break;a=a.parentNode}}
var k="search/logger.php?id="+id+"&path="+path;a.href=k;a.onmousedown=""}
catch(l){}
return true};if(typeof(bsn)=="undefined")
_b=bsn={};if(typeof(_b.Autosuggest)=="undefined")
_b.Autosuggest={};_b.AutoSuggest=function(id,param)
{if(!document.getElementById)
return 0;var ua=navigator.userAgent;var index=ua.indexOf('MSIE');if(index!=-1){var ie_version=parseFloat(ua.substring(index+'MSIE'.length+1))
if(ie_version<=6){this.bad_IE=true;}else{this.bad_IE=false;}}else{this.bad_IE=false;}
this.fld=_b.DOM.gE(id);if(!this.fld)
...
{var pointer=this;_b.DOM.remE(this.idAs);this.killTimeout();if(arr.length==0&&!this.oP.shownoresults&&this.bad_IE==false)
return false;var div=_b.DOM.cE("div",{id:this.idAs,className:this.oP.className});var hcorner=_b.DOM.cE("div",{className:"as_corner"});var hbar=_b.DOM.cE("div",{className:"as_bar"});var header=_b.DOM.cE("div",{className:"as_header"});header.appendChild(hcorner);header.appendChild(hbar);div.appendChild(header);var ul=_b.DOM.cE("ul",{id:"as_ul"});if(this.fld.id=='ajaxfield_main'&&this.bad_IE==true){switch(arr.length){case 1:document.getElementById("article_selector").style.visibility='visible';break;case 0:document.getElementById("article_selector").style.visibility='visible';return 0;break;default:document.getElementById("article_selector").style.visibility='hidden';break;}}
for(var i=0;i<arr.length;i++)
{var val=arr[i].value;var st=0;var output=val.substring(0,st)+"<em>"+val.substring(st,st+this.sInp.length)+"</em>"+val.substring(st+this.sInp.length);var span=_b.DOM.cE("span",{},output,true);if(arr[i].info!="")
{var br=_b.DOM.cE("br",{});span.appendChild(br);var small=_b.DOM.cE("small",{},arr[i].info);span.appendChild(small);}
var a=_b.DOM.cE("a",{href:"#"});var tl=_b.DOM.cE("span",{className:"tl"}," ");var tr=_b.DOM.cE("span",{className:"tr"}," ");a.appendChild(tl);a.appendChild(tr);a.appendChild(span);a.name=i+1;a.onclick=function(){pointer.setHighlightedValue();return false;};a.onmousemove=function(){pointer.setHighlight(this.name);};var li=_b.DOM.cE("li",{},a);ul.appendChild(li);}
...
this.oP.callback(this.aSug[this.iHigh-1]);this.fld.form.submit();}};_b.AutoSuggest.prototype.killTimeout=function()
...
{this.killTimeout();var ele=_b.DOM.gE(this.idAs);var pointer=this;document.getElementById("article_selector").style.visibility='visible';if(ele)
